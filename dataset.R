...
#' }
#'
ukb_db <- function(fileset, path = ".", dbname="duck.db", mapping = NULL, stata=TRUE,
  ...
  for(row in seq(1, 21, by=1)){
    ...
    if(row==1){
      ...
      bd <- read_ukb_tab(str_replace(tab_location, pattern=".tab", replacement=paste0("_", sprintf("%02d", row-1), ".tab")),
                         column_type = ukb_key$fread_column_type,
                         # skip=row,
